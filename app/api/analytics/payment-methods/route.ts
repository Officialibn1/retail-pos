import { NextRequest, NextResponse } from "next/server";
import { requireAuth, requireManager } from "@/lib/middleware/auth";
import { getPaymentMethodBreakdown } from "@/lib/services/analytics.service";

export const dynamic = 'force-dynamic';

/**
 * GET /api/analytics/payment-methods
 * Get aggregated sales data by payment method
 * Requires MANAGER+ role
 */
export async function GET(request: NextRequest) {
	// Authenticate user
	const authResult = await requireAuth(request);
	if (authResult instanceof NextResponse) {
		return authResult;
	}

	const { request: authenticatedRequest } = authResult;
	const user = authenticatedRequest.user;

	// Check role permissions
	const roleCheck = requireManager()(authenticatedRequest);
	if (roleCheck) {
		return roleCheck;
	}

	try {
		// Get payment method breakdown with role-based filtering
		const paymentMethods = await getPaymentMethodBreakdown(user.id, user.roles);

		return NextResponse.json(paymentMethods, { status: 200 });
	} catch (error: any) {
		console.error("Error getting payment method breakdown:", error);

		return NextResponse.json(
			{
				error: {
					message: "Failed to retrieve payment method breakdown",
					code: "INTERNAL_ERROR",
				},
			},
			{ status: 500 },
		);
	}
}
