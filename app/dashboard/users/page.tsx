...
export default function UsersPage() {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      ...
      case "SalesPerson":
        return <Badge className="bg-lunar-green-100 text-lunar-green-800 hover:bg-lunar-green-100">Sales Person</Badge>
      default:
  ...
  const getShiftBadge = (shift: string) => {
    return (
      <Badge
        variant="outline"
        className={shift === "Morning" ? "border-orange-300 text-orange-700" : "border-purple-300 text-purple-700"}
      >
        {shift}
      </Badge>
    )
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-lunar-green-800">User Management</h1>
          <p className="text-lunar-green-600 mt-1">Manage store users and their permissions</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border-lunar-green-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-lunar-green-700">Total Users</CardTitle>
            <Users className="h-4 w-4 text-lunar-green-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-lunar-green-800">{totalUsers}</div>
            <p className="text-xs text-lunar-green-600">{activeUsers} active users</p>
          </CardContent>
        </Card>

        <Card className="border-lunar-green-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-lunar-green-700">Morning Shift</CardTitle>
            <Users className="h-4 w-4 text-lunar-green-600" />
          </CardHeader>
          <CardContent>
            ...
            <p className="text-xs text-lunar-green-600">users on morning shift</p>
          </CardContent>
        </Card>

        <Card className="border-lunar-green-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-lunar-green-700">Night Shift</CardTitle>
            <Users className="h-4 w-4 text-lunar-green-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-lunar-green-800">
      ...
      <Card className="border-lunar-green-200">
        ...
        <CardContent>
            ...
            <TableBody>
              {filteredUsers.map((u) => (
                <TableRow key={u.id} className="border-lunar-green-100">
                  <TableCell className="font-medium text-lunar-green-800">{u.name}</TableCell>
                  <TableCell className="text-lunar-green-700">{u.email}</TableCell>
                  ...
                  <TableCell className="text-lunar-green-700">{u.createdAt.toLocaleDateString()}</TableCell>
                  <TableCell>
                    <div className="flex gap-1">
                      <Button size="sm" variant="ghost" className="text-lunar-green-600 hover:bg-lunar-green-100">
          ...
          {filteredUsers.length === 0 && (
            <div className="text-center py-8 text-lunar-green-600">No users found matching your criteria.</div>
          )}
        </CardContent>
      </Card>
    </div>
