...
export default function NewSalePage() {
  ...
  const addToCart = (product: InventoryItem, quantity: number) => {
    const existingItem = cartItems.find((item) => item.inventoryItemId === product.id)
  ...
  const completeSale = (paymentMethod: "cash" | "card" | "digital", shouldShowReceipt = true) => {
    if (!user) return
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-lunar-green-600 mt-1">Create a new sales transaction</p>
        </div>
        <Button
          onClick={clearCart}
          variant="outline"
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <Card className="border-lunar-green-200">
            <CardHeader>
              <CardTitle className="text-lunar-green-800">Select Products</CardTitle>
            </CardHeader>
            <CardContent>
      ...
      <ReceiptPrintDialog sale={completedSale} open={showReceipt} onOpenChange={setShowReceipt} />
    </div>
  )
}
