...
export async function createSale(data: CreateSaleInput): Promise<Sale> {
  ...
  // ── Validate promotion code upfront (outside transaction) ──────────────
  ...
  if (data.promotionCode) {
    ...
    const promoResult = await validatePromoCode({
      ...
      items: data.items.map((i) => ({
        ...
        categoryId: invMap.get(i.inventoryItemId)?.categoryId ?? "",
        price: i.price,
        quantity: i.quantity,
      })),
    });
    ...
    resolvedPromoId = promoResult.promotion.id;
    promoDiscountAmount =
      data.promotionDiscountAmount != null
        ? data.promotionDiscountAmount
  ...
  const result = await prisma.$transaction(
    async (tx) => {
      ...
      const manualDiscountAmount = (subtotal * data.discountRate) / 100;
      const totalDiscountAmount = manualDiscountAmount + promoDiscountAmount;
      ...
      // Increment promotion usage atomically inside the transaction
      if (resolvedPromoId) {
        await incrementPromotionUsage(resolvedPromoId, tx);
      }
