...
interface ShoppingCartProps {
  ...
  onApplyPromo: (promo: AppliedPromotion) => void;
  onRemovePromo: () => void;
}
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-brand-main-700">
                  <span>Subtotal:</span>
                  <span>{formatNaira(subtotal, c)}</span>
                </div>
                {discount > 0 && (
                  <div className="flex justify-between text-brand-main-700">
                    <span>Discount ({discount}%):</span>
                    <span>-{formatNaira(manualDiscountAmount, c)}</span>
                  </div>
                )}
                {appliedPromo && (
                  <div className="flex justify-between text-green-700">
                    <span>Promo ({appliedPromo.code}):</span>
                    <span>-{formatNaira(promoDiscountAmount, c)}</span>
                  </div>
                )}
                <div className="flex justify-between text-brand-main-700">
                  <span>Tax ({Number(taxRate) * 100}%):</span>
                  <span>{formatNaira(taxAmount, c)}</span>
                </div>
                <Separator className="bg-brand-main-200" />
                <div className="flex justify-between font-medium text-brand-main-800">
                  <span>Total:</span>
                  <span>{formatNaira(total, c)}</span>
                </div>
              </div>
